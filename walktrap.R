...
communities <- cluster_walktrap(G, weight, steps = 2)
# Apply walktrap algorithm for community detection

# Calculate conductance for each community
conductance_score_walktrap <- conductance(G, communities$membership)

# Create a data frame
df <- data.frame(Community = 1:length(conductance_scores), Conductance = conductance_scores)

# Write to CSV
write.csv(df, "conductance_scores_walktrap.csv", row.names = FALSE)
...
for (i in 1:length(communities)) {
  ...
  community_dict_walktrap[[paste0("Community_", i)]] <- V(G)$name[community_nodes]
}
...
# Print NMI
cat("NMI:", nmi, "\n")
print(community_dict_walktrap)
# Convert the list to a character string
community_dict_walktrap_str <- toString(community_dict_walktrap)

# Write the string to a .txt file
writeLines(community_dict_spinglass_str, "community_dict_walktrap.txt")
