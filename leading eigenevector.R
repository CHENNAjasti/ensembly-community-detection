...
communities <- cluster_leading_eigen(G, weight, steps = 12)
...
# Write to CSV
write.csv(df, "conductance_scores_eigen.csv", row.names = FALSE)
...
for (i in 1:length(communities)) {
  ...
  community_dict_eigen[[paste0("Community_", i)]] <- V(G)$name[community_nodes]
}
...
# Print NMI
cat("NMI:", nmi, "\n")
print(community_dict_eigen)
# Convert the list to a character string
community_dict_eigen_str <- toString(community_dict_eigen)

# Write the string to a .txt file
writeLines(community_dict_eigen_str, "community_dict_eigen.txt")
