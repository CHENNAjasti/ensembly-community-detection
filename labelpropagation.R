...
communities <- cluster_label_prop(G, weight)
...
# Write to CSV
write.csv(df, "conductance_scores_label.csv", row.names = FALSE)
...
for (i in 1:length(communities)) {
  ...
  community_dict_label[[paste0("Community_", i)]] <- V(G)$name[community_nodes]
}
...
# Print NMI
cat("NMI:", nmi, "\n")
print(community_dict_label)
# Convert the list to a character string
community_dict_label_prop_str <- toString(community_dict_label)

# Write the string to a .txt file
writeLines(community_dict_label_prop_str, "community_dict_label.txt")
