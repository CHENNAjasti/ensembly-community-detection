...
communities <- cluster_spinglass(G, weight, spins = 20)

# Calculate conductance for each community
conductance_scores <- conductance(G, communities$membership)

# Create a data frame
df <- data.frame(Community = 1:length(conductance_scores), Conductance = conductance_scores)

# Write to CSV
write.csv(df, "conductance_scores_spinglass.csv", row.names = FALSE)
...
for (i in 1:length(communities)) {
  ...
  community_dict_spinglass[[paste0("Community_", i)]] <- V(G)$name[community_nodes]
}
...
# Print NMI
cat("NMI:", nmi, "\n")
print(community_dict_spinglass)
# Convert the list to a character string
community_dict_spinglass_str <- toString(community_dict_spinglass)

# Write the string to a .txt file
writeLines(community_dict_spinglass_str, "community_dict_spinglass.txt")
