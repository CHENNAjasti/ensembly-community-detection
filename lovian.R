...
communities <- cluster_louvain(G, weight,resolution = 1.34)
...
# Write to CSV
write.csv(df, "conductance_scores_lovian.csv", row.names = FALSE)
...
for (i in 1:length(communities)) {
  ...
  community_dict_lovian[[paste0("Community_", i)]] <- V(G)$name[community_nodes]
}
...
# Print NMI
cat("NMI:", nmi, "\n")
print(community_dict_lovian)
# Convert the list to a character string
community_dict_lovian_str <- toString(community_dict_lovian)

# Write the string to a .txt file
writeLines(community_dict_lovian_str, "community_dict_lovian.txt")
