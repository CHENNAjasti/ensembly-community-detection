...
communities <- cluster_infomap(G, weight, modularity = 0.5)
...
# Write to CSV
write.csv(df, "conductance_scores_infomap.csv", row.names = FALSE)
...
for (i in 1:length(communities)) {
  ...
  community_dict_infomap[[paste0("Community_", i)]] <- V(G)$name[community_nodes]
}
...
# Print NMI
cat("NMI:", nmi, "\n")
print(community_dict_infomap)
# Convert the list to a character string
community_dict_infomap_str <- toString(community_dict_infomap)

# Write the string to a .txt file
writeLines(community_dict_infomap_str, "community_dict_infomap.txt")
